		    	...
		    	if(result.statusCode==200){
		    		$.each(result.data, function(idx, obj){	
		    			if(obj.offerType=="value"){
		    				if(obj.discountType=="Tk"){
				    			  discount+=obj.discountAmount*received_qty;
				    			  netDiscount+=parseFloat(obj.discountAmount*received_qty);	
					    		}else{
					    			 discount+=((totalPrice*obj.discountAmount)/100);
					    			 netDiscount+=parseFloat(discount);
					    		}		
		    		...
		    		$.each(result, function(idx, obj){		    			
	    				if(netTotal>=parseFloat(obj.requiredInvoiceAmount)){
	    					...
	    					if(obj.discountType=="Percentage") {
	  	    				   ...
	  	    				   var m = parseFloat(obj.discountAmount);
	  	    				   var net = (netTotal*m)/100;
	  	    				   netDiscount+= net;
...
function formClear() {
	...
	function productListSubmitToServer() {
		...
		distributorRequisition["deliveryStatus"]="Not Delivered";
...
//	function allOrderedproductListSubmitToServer() {
//		this.event.preventDefault();
//		
//		var today = new Date();
//		var date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
...
//	
	...
	function allOrderedproductListSubmitToServer2() {
		this.event.preventDefault();
		...
		let order= {};		
		var uniqueNumber= Math.floor(new Date().valueOf() * Math.random());
		//order["dealerId"]=$("#distributor_id").val();
		order["orderNumber"]=uniqueNumber;
		//order["productWiseOfferId"]=$("#productwiseOfferId").text();
		//order["cashBackId"]=$("#cashBackOfferId").text();
		order["totalPrice"]=$("#netTotal").text();
		order["netDiscount"]=$("#netDiscount").text();
		order["grandTotal"]=$("#netGrandTotal").text();
	    ...
	    $(T).find('> tbody > tr').each(function () {	    	
	    	let orderedProduct= {};
	    	orderedProduct["productId"]=$(this).find("td:eq(0)").text();
	    	orderedProduct["productName"]=$(this).find("td:eq(1)").text();
	    	orderedProduct["receivedQty"]=$(this).find("td:eq(2)").text();
	    	orderedProduct["productUnitPrice"]=$(this).find("td:eq(3)").text();
	    	orderedProduct["totalPrice"]=$(this).find("td:eq(4)").text();
	    	orderedProduct["discount"]=$(this).find("td:eq(5)").text();
	    	orderedProduct["grandTotal"]=$(this).find("td:eq(6)").text();
	    	orderProductList.push(orderedProduct);
	    	
	        }); 
	    
	    order["OrderedProducts"]=orderProductList;
	    
	    if(orderProductList.length>0){
	    	submitToServer2(order);
	    ///	submitEditFormToServer3(order);
	    } 
	    else {
	    	alert("Please Enter at least one product!");
	...
	function orderedProductListConfirmSubmitToServer() {
		this.event.preventDefault();
		
		let orderEcommerceModel= {};		
		var uniqueNumber= Math.floor(new Date().valueOf() * Math.random());

		orderEcommerceModel["buiyerId"]=$("#dealerId").text();
		orderEcommerceModel["orderNumber"]=uniqueNumber;
//		distributorRequisition["productWiseOfferId"]=$("#productwiseOfferId").text();
//		distributorRequisition["cashBackId"]=$("#cashBackOfferId").text();
		orderEcommerceModel["totalPrice"]=$("#netTotal").text();
		orderEcommerceModel["netDiscount"]=$("#netDiscount").text();
		orderEcommerceModel["grandTotal"]=$("#netGrandTotal").text();
	//	distributorRequisition["deliveryStatus"]="Not Delivered";
		orderEcommerceModel["status"]="0";
		
		 var T = document.getElementById('productTable');
		var orderedProductList=[];
	    $(T).find('> tbody > tr').each(function () {	    	
	    	let orderProduct= {};
	    	orderProduct["productId"]=$(this).find("td:eq(0)").text();
	    	orderProduct["productName"]=$(this).find("td:eq(1)").text();
	    	orderProduct["receivedQty"]=$(this).find("td:eq(2)").text();
	    	orderProduct["productUnitPrice"]=$(this).find("td:eq(3)").text();
	    	orderProduct["totalPrice"]=$(this).find("td:eq(4)").text();
	    	orderProduct["discount"]=$(this).find("td:eq(5)").text();
	    	orderProduct["grandTotal"]=$(this).find("td:eq(6)").text();
	    	orderedProductList.push(orderProduct);
...
//	    //alert(JSON.stringify(freeProductList));
	   // distributorRequisition["freeProductlist"]=freeProductList;
	    ...
	    orderEcommerceModel["orderedProducts"]=orderedProductList;
	    if(orderedProductList.length>0){
	    	 submitOrderToServer2(orderEcommerceModel);	
	    }else{
	    	alert("Please Enter at least one product!");
	    }
	}
	
	function submitOrderToServer2(data) {
		
		//alert("hi")

	    let url = baseUrl +'/api/save-order-ecommerce'
	    $.ajax({
	        type: "POST",
	        ...
	        dataType : "json",
	        success: function (result) {
	    	 	swal(`Thanks for confirming Order`);
	        	//alert("Thanks for confirming requisition\nPlease wait for further process.");	            
	            window.location.href=baseUrl+"/admin/order-ecommerce-home";
	        },
	        error: function (e) {
	            console.log("ERROR: ", e);
	        }
	...
	function submitToServer(data) {
	    ...
	    $.ajax({
	        ...
	        dataType : "json",
	        success: function (result) {
	    	 	swal(`Thanks for confirming requisition\nPlease wait for further process.`);
	        	//alert("Thanks for confirming requisition\nPlease wait for further process.");	            
	            window.location.href=baseUrl+"/admin/confirmed-requisition";
	        },
	        error: function (e) {
	            console.log("ERROR: ", e);
	        }
	...
	function submitToServer2(data) {
	    ...
	    $.ajax({
	        ...
	        success: function (result) {
	    	 	swal(`Thanks for confirming requisition\nPlease wait for further process.`);
	    	 	//swal(`Please confirm your order`);
	        	//alert("Thanks for confirming requisition\nPlease wait for further process.");	            
	            //window.location.href=baseUrl+"/admin/confirmed-requisition";
	    	 	//		$("#confirmOrdeDiv").attr("hidden",false);
	        },
	        error: function (e) {
	            console.log("ERROR: ", e);
	        }
...
function submitEditFormToServer3(data) {
	...
	//	 alert(received_qty+" "+stock);
	        if(parseInt(received_qty) > parseInt(stock)) {
	        	...
	        	swal(`required quantity cross stock quantity.`);
				    ...
				    success:function(result) {				    	
				    	if(result.statusCode==200){
				    		if(result.data[0].offerType=="value"){
					    		if(result.data[0].discountType=="Tk"){
					    			  discountVal=result.data[0].discountAmount*received_qty;
					    			  var grandTotalvalue =total-discountVal;
					    			  tt.closest('td').siblings().find('#discount').val(discountVal);
							    	   tt.closest('td').siblings().find('#grandTotal').val(grandTotalvalue);
							    	   tt.closest('td').siblings().find('#totalPrice').val(total);
							    	   sumTotal();
					    			
						    		}
					    		else {
						    			 discountVal=((total*result.data[0].discountAmount)/100);	
						    			 var grandTotalvalue =total-discountVal;
...
//        alert(totalValue+" "+discountValue+" "+grandTotalValue);
        ...
        setNetTotalAmountSecond(discountValue,totalValue,grandTotalValue);	   
		}
...
function submitEditFormToServer(data,id) {
	    ...
	    $.ajax({
	        ...
	        success: function (result) {
	        	//alert("Requisition Approved successful.");
	    	 	swal(`Requisition Approved successful.`);
	...
	function productListSubmitToServerByWarhouse() {
	    ...
	    $(T).find('> tbody > tr').each(function () {	    	
	    	...
	    	distributorProduct["productName"]=$(this).find("td:eq(1)").find('input').val();
	    	distributorProduct["receivedQty"]=$(this).find("td:eq(2)").find('input').val();
//	    	distributorProduct["receivedQty"]=$(this).find("td:eq(10)").find('input').val();
	    	distributorProduct["productUnitPrice"]=$(this).find("td:eq(6)").find('input').val();
	    	distributorProduct["totalPrice"]=$(this).find("td:eq(7)").find('input').val();
	    	distributorProduct["discount"]=$(this).find("td:eq(8)").find('input').val();
	    	distributorProduct["grandTotal"]=$(this).find("td:eq(9)").find('input').val();
		 ...
		 $.ajax({
	         ...
	         success: function (result) {
	          //  alert("Requisition forward succesful");
	            swal(`Requisition Forwarded successful.`);
	            window.history.back();
	         },
	         error: function (e) {
	             console.log("ERROR: ", e);
...
//		    	distRequisistionReceive["receivedQty"]=$(this).find("td:eq(7)").find('input').val();
...
function deleteEntry(id) {		
	let url = baseurl+"/api/delete-distributor-requisition/"+id;
...
function setNetTotalAmountSecond(netDiscount,netTotal,netGrandTotal){
    $.ajax({
    url: baseUrl+'/api/get-available-cash-back-campaign',
    type: "GET",
    dataType: "json",
...
function submitOrderForm() {
    ...
    let order = {};
    order["orderNumber"] = $("#order_num").val();  
    submitOrderFormToServer(order);
}


function submitOrderFormToServer(data) {
    ...
    $.ajax({
        ...
        success: function (result) {
    	swal(`${result.message}`);
           window.location.href =  baseUrl +'/admin/order-ecommerce';
...
function rejectOrderEcom() {
	this.event.preventDefault();
	
   
}

function approveOrderEcom() {
	this.event.preventDefault();
